...
input int            SessionsToCount         = 9;               // SessionsToCount: Number of sessions to count Market Profile.
...
input color          KeyValuesColor           = clrNONE;       // KeyValuesColor: color for VAH, VAL, POC printout.
...
input int  IntradaySessionMapShiftMinutes               = 0;
...
int OnInit()
{
    ...
    else if (Session == Intraday && EnableIntradaySession)
    {
        ...
        {
            // Apply time shift to start and end times
            string shiftedStartTime = ShiftTimeString(startTimes[i], IntradaySessionMapShiftMinutes);
            string shiftedEndTime = ShiftTimeString(endTimes[i], IntradaySessionMapShiftMinutes);

            if (!CheckIntradaySession(EnableIntradaySession, shiftedStartTime, shiftedEndTime, IntradaySessionColorScheme)) {
...
//+------------------------------------------------------------------+
//| Shifts a time string by specified minutes.                       |
//| Returns shifted time in "HH:MM" format.                          |
//+------------------------------------------------------------------+
string ShiftTimeString(string time_string, int shift_minutes)
{
    int hours, minutes, total_minutes;
    if (!GetHoursAndMinutes(time_string, hours, minutes, total_minutes)) {
    ...
    hours = (int)MathFloor(total_minutes / 60.0);
